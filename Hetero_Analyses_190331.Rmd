---
title: "Aggregated Code and Analyses for Heterogeneity Paper, *JEPG*, 2019"
...
newvary <- expand.grid(valenceE = c(-.5, .5), id = c(208, 222, 241, 239, 247))
...
to_string <- as_labeller(c(`208` = paste(unique(subset(newvary, id == 208)$sloperound)), 
                           `239` = paste(unique(subset(newvary, id == 239)$sloperound)),
...
valrt_panelplot_ids
#ggsave(valrt_panelplot_ids, file = "Fig2_valrt_panelplotbyranef_subIDs_190303.pdf", height = 4, width = 8)
...
```{r fo bayesian model, echo = T, results = "hide"}
...
```{r math priming bayesian model, echo = T, results = "hide"}
...
```{r math priming bayesian model int only, results = "hide"}
...
```{r promotion bayesian model, echo = T, results = "hide"}
...
```{r temp stability bayesian, echo = T, results = "hide"}
...
```{r temp stability bayesian cor is 1, results = "hide"}
...
```{r}
manip <- read.csv("heterogeneity_dataset4_temporalstability_rfcondition.csv")
```
...
# Package Version Information & Notes

Note: These analyses were performed using lme4 version 1.1-15, as noted below. Due to package changes, there may be a few analyses that throw false convergence warnings with later versions of lme4 (e.g., lme4 1.1-20). To circumvent the false convergence issues, some models may need to be rerun with an earlier optimizer (visit [this page](https://stats.stackexchange.com/questions/400479/convergence-issues-with-lme4-1-1-20-for-models-that-converged-when-using-earlier) for details). To change the optimizer, run the following, replacing `lmer_object` with the name of the model with convergence issues and `lmer_object_new` with a name of your choosing for the updated version:

```{r, eval = F}
lmer_object_new <- update(lmer_object, control=lmerControl(optimizer="bobyqa"))
summary(logRTboth_manip2)
```
